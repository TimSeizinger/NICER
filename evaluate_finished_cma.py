...
if not "batch_data.csv" in os.listdir(in_dir):
    yaml_dir = Path("k8s") / Path("hyperparametersearch_cma")
...
if not os.path.isdir(Path('analysis/results/hyperparametersearch_cma')):
    os.mkdir(Path('analysis/results/hyperparametersearch_cma'))
...
for i in range(10):
    ...
    results = {'yaml_name': yaml_names, 'mean_distance_to_orig': mean_distances,
               'sigma': sigmas, 'gamma': gammas, 'magrin': margins, 'composite_balance': composite_balances,
