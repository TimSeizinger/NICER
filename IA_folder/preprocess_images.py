...
mapping["composition"]["crop_leftcorners"] = ["pos", "neg"]
...
class ImageEditor:
    def __init__(self):
        ...
        self.style_intensity_mapping = {
            "brightness_pos": [float(x) for x in np.linspace(0, 0.6, 6)],
            "brightness_neg": [float(x) for x in np.linspace(0, -0.6, 6)],
            "contrast_pos": [float(x) for x in np.linspace(1, 1.5, 6)],
            "contrast_neg": [float(x) for x in np.linspace(1, 0.5, 6)],
            "shadows_pos": [float(x) for x in np.linspace(1, 80, 6)],
            "shadows_neg": [float(x) for x in np.linspace(1, -80, 6)],
            "highlights_pos": [float(x) for x in np.linspace(1, 80, 6)],
            "highlights_neg": [float(x) for x in np.linspace(1, -80, 6)],
            "exposure_pos": [float(x) for x in np.linspace(0, 5, 6)],
            "exposure_neg": [float(x) for x in np.linspace(1, -5, 6)],
            "temperature_pos": [float(x) for x in np.linspace(6500, 12000, 6)],
            "temperature_neg": [float(x) for x in np.linspace(6500, 2000, 6)],
    ...
    def _crop(self, img: Image.Image, intensity_h: int = 0, intensity_v: int = 0) -> Image.Image:
        crop_size = transforms.Resize(224)(img).size
        ...
        logging.debug(f"s({crop_size[0]},{crop_size[1]});{center_left},{center_top},{center_right},{center_bottom}")
        ...
        with NamedTemporaryFile(suffix=".jpg") as src_file:
            ...
            for distortion, intensity in distortion_intens_tuple_list:
                ...
                if distortion == "original":
                    ...
                    with NamedTemporaryFile(suffix=".jpg") as out_file:
